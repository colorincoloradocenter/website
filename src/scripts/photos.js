export function initPhotos() {
    ...
    mediaFiles.forEach((fileName, idx) => {
        ...
        const blurButton = `<button class="media-blur-btn">${buttonLabels[idx] || ""}</button>`;

        const mediaCard = `
            <div class="media-card">
                ${mediaTag}
                <div class="media-blur-text">${blurButton}</div>
            </div>`;
    ...
    const modalContents = [
        `Hola`,
        `
        <form id="servicios-form" style="display: flex; flex-direction: column; gap: 1rem;">
            <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                ${servicios.map((serv, i) => `
                    <label style="display: flex; align-items: flex-start; gap: 0.5em; cursor: pointer;">
                        <input type="radio" name="servicio" value="${serv}" ${i === 0 ? "checked" : ""} style="margin-top: 0.2em;">
                        <span>${serv.replace(/([A-ZÁÉÍÓÚÑ][a-záéíóúñ]+)(?=\s|\(|$)/g, "<b>$1</b>")}</span>
                    </label>
                `).join("")}
            </div>
            <button type="submit" id="btn-interesa" style="margin-top:1.5rem; background:#0142c0; color:#fff; border:none; border-radius:45px; padding:0.7em 1.5em; font-size:1rem; font-weight:600; cursor:pointer;">
                Me interesa
            </button>
        </form>
        `,
